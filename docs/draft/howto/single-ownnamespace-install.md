...
`spec.config.inline.watchNamespace: <namespace>` field. The *installMode* is inferred in the following way:

 - *AllNamespaces*: `<namespace>` is empty, or the configuration is not present
